...
const char *
np_error_get_text(unsigned int type, gboolean *debug)
{
	static char msg[256];
	const char *result;
	*debug = FALSE;
...
void
np_error_handle(NPSession *session, unsigned int type)
{
	char *buf;
	gboolean debug;
    
	buf = g_strdup_printf(_("NP Error: %s\n"),
	                      np_error_get_text(type, &debug));
	if (debug)
		purple_debug_warning("np", "error %d: %s\n", type, buf);
	else
		purple_notify_error(session->account->gc, NULL, buf, NULL);
	g_free(buf);
}

/* Remove the buddy referenced by the NPAddRemData before the serverside list
 * is changed.  If the buddy will be added, he'll be added back; if he will be
 * removed, he won't be. */
/* Actually with our NPP14 code that isn't true yet, he won't be added back :(
 * */
static void
np_complete_sync_issue(NPAddRemData *data)
{
	PurpleBuddy *buddy;
...
void
np_error_sync_issue(NPSession *session, const char *passport,
                     const char *group_name)
{
	...
	purple_request_action(gc, NULL, msg, reason, PURPLE_DEFAULT_ACTION_NONE,
                          ...
                          data, 2,
                          _("Yes"), G_CALLBACK(np_add_cb),
                          _("No"), G_CALLBACK(np_rem_cb));
