/**
 * @file nputils.c Utility functions
 *
 * purple
 *
...
void
np_parse_format(const char *mime, char **pre_ret, char **post_ret)
{
	char *cur;
 ...
 * and converts it to np formatting. It doesn't deal with the tag closing,
 ...
 */
void
np_import_html(const char *html, char **attributes, char **message)
{
	int len, retcount = 0;
	const char *c;
	char *msg;
	...
	np_encode_spaces(fontface, fontface_encoded, BUF_LEN);
...
void
np_parse_socket(const char *str, char **ret_host, int *ret_port)
{
	char *host;
	char *c;
	int port;
...
void
np_parse_user(const char *str, char **ret_user, int *ret_network)
{
	char **tokens;
...
gboolean
np_email_is_valid(const char *passport)
{
	if (purple_email_is_valid(passport)) {
		...
		while (*passport != '@') {
			if (*passport == '/')
				...
				return FALSE;
			/* np also doesn't like colons, but that's checked already */
...
/***************************************************************************
 * np Challenge Computing Function
 ***************************************************************************/
...
#define BUFSIZE	256
void
np_handle_chl(char *input, char *output)
{
	PurpleCipher *cipher;
	PurpleCipherContext *context;
//	const guchar productKey[] = npP15_WLM_PRODUCT_KEY;
//	const guchar productID[]  = npP15_WLM_PRODUCT_ID;
...
guint8
np_read8(const char *buf)
{
	return (guint8)buf[0];
}

guint16
np_read16le(const char *buf)
{
	return GUINT16_FROM_LE(*(guint16 *)buf);
}

guint16
np_read16be(const char *buf)
{
	return GUINT16_FROM_BE(*(guint16 *)buf);
}

guint32
np_read32le(const char *buf)
{
	return GUINT32_FROM_LE(*(guint32 *)buf);
}

guint32
np_read32be(const char *buf)
{
	return GUINT32_FROM_BE(*(guint32 *)buf);
}

guint64
np_read64le(const char *buf)
{
	return GUINT64_FROM_LE(*(guint64 *)buf);
}

guint64
np_read64be(const char *buf)
{
	return GUINT64_FROM_BE(*(guint64 *)buf);
}

void
np_write8(char *buf, guint8 data)
{
	*(guint8 *)buf = data;
}

void
np_write16le(char *buf, guint16 data)
{
	*(guint16 *)buf = GUINT16_TO_LE(data);
}

void
np_write16be(char *buf, guint16 data)
{
	*(guint16 *)buf = GUINT16_TO_BE(data);
}

void
np_write32le(char *buf, guint32 data)
{
	*(guint32 *)buf = GUINT32_TO_LE(data);
}

void
np_write32be(char *buf, guint32 data)
{
	*(guint32 *)buf = GUINT32_TO_BE(data);
}

void
np_write64le(char *buf, guint64 data)
{
	*(guint64 *)buf = GUINT64_TO_LE(data);
}

void
np_write64be(char *buf, guint64 data)
{
